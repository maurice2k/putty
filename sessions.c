...
static char* session_name_cleanup(char* name)
{
    ...
    for (i = 0; i < strlen(name); ++i) {
        if (name[i] == ' ') {
            ...
            if (ws == i - (lastdc + 1)) {
                // we have yet another double colon with nothing or
                // space in between, skip it
                skip++;
            }
            lastdc = i;
            skip += ws;
...
{
    session_tree *sess_tree = snew(session_tree);
    sess_tree->nodes = 0;
    ...
    session_node *reference_sibling, char *name, int index)
{
   ...
   /* if (parent->first_child == NULL) {
    ...
    if (child == reference_sibling) {
        ...
        // insert before reference sibling (or at the end if
        //reference_sibling is NULL)
        while (child->next_sibling != reference_sibling &&
...
// inserts node based on name in sorted order. if a node with the same name
// already exists no new node is inserted and the existing node is being
// returned. otherwise the new node is returned.
session_node *session_node_insert_sorted_unique_name(session_node *parent,
    char *name, int index)
{
...
{
    session_node *sess_node = snew(session_node);
    sess_node->name = snewn(strlen(name) + 1, char);
...
void session_node_free(session_tree *sess_tree, session_node *sess_node)
{
    ...
    if (sess_node->user_data != NULL &&
        sess_tree->node_user_data_free_callback) {
            (*sess_tree->node_user_data_free_callback)(sess_node->user_data,
                sess_node);
    }

    sfree(sess_node->name);
    sfree(sess_node);
    sess_node = NULL;
}
    ...
    int index)
{
    ...
    name = snewn(strlen(flat_name) + 1, char);
...
void *session_node_traverse(session_node *root_node,
    ...
    while (current != NULL) {
        if ((res = (*traverse_callback)(current, root_node,
            current->first_child == NULL, level, extra_data)) != NULL) {
            return res;
        }
        level++;
        if ((res = session_node_traverse(current, traverse_callback,
            extra_data)) != NULL) {
            return res;
        }
        level--;
...
void session_tree_reload_from_sesslist(session_tree **sess_tree,
...
static void *_session_node_find_cb(session_node *current, session_node *parent,
...
session_node *session_tree_find_leaf_node_by_index(session_tree *sess_tree,
    int index)
{
    return session_node_traverse(sess_tree->root_node, _session_node_find_cb,
        &index);
}
